package co.thebabels.schedulecalendarview.app

import android.content.Context
import android.graphics.Canvas
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.view.View
import android.view.ViewGroup
import android.widget.TextView
import androidx.recyclerview.widget.PagerSnapHelper
import androidx.recyclerview.widget.RecyclerView
import co.thebabels.schedulecalendarview.ScheduleCalendarAdapter
import co.thebabels.schedulecalendarview.ScheduleCalendarLayoutManager
import co.thebabels.schedulecalendarview.ScheduleItem
import java.util.*

class MainActivity : AppCompatActivity() {

    private val adapter = TextScheduleAdapter()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)

        val recyclerView: RecyclerView = findViewById(R.id.recycler_view)
        recyclerView.layoutManager = ScheduleCalendarLayoutManager(this)
        recyclerView.adapter = adapter
//        val snapHelper = PagerSnapHelper()
//        snapHelper.attachToRecyclerView(recyclerView)


        val cal = Calendar.getInstance()
        adapter.addItems(
            TextScheduleItem("text-000", cal.time, cal.apply { add(Calendar.HOUR, 1) }.time),
            TextScheduleItem(
                "text-001",
                cal.apply { add(Calendar.HOUR, 1) }.time,
                cal.apply { add(Calendar.HOUR, 1) }.time
            ),
            TextScheduleItem(
                "text-002",
                cal.apply { add(Calendar.HOUR, 1) }.time,
                cal.apply { add(Calendar.HOUR, 1) }.time
            ),
            TextScheduleItem(
                "text-003",
                cal.apply { add(Calendar.HOUR, 1) }.time,
                cal.apply { add(Calendar.HOUR, 1) }.time
            ),
        )

    }

    data class TextScheduleItem(val text: String, val start: Date, val end: Date) : ScheduleItem {
        override fun start(): Date {
            return this.start
        }

        override fun end(): Date {
            return this.end
        }
    }


    class TextScheduleAdapter() : ScheduleCalendarAdapter<TextScheduleItem>() {

        override fun createScheduleViewHolder(parent: ViewGroup): ScheduleCalendarAdapter.ViewHolder<TextScheduleItem> {
            return ViewHolder(TextView(parent.context))
        }

        class ViewHolder(itemView: View) :
            ScheduleCalendarAdapter.ViewHolder<TextScheduleItem>(itemView) {

            override fun bind(item: TextScheduleItem) {
                if (itemView is TextView) {
                    itemView.text = item.text
                }
            }
        }
    }
}